...
function UnitsTab() {
  const units = [
    { id: 'MED-1', type: 'Ambulance', status: 'Available', location: 'Station 1' },
    { id: 'MED-2', type: 'Ambulance', status: 'Dispatched', location: '123 Main St' },
    { id: 'ENG-1', type: 'Fire Engine', status: 'En Route', location: '456 Oak Ave' },
    { id: 'LAD-1', type: 'Ladder', status: 'On Scene', location: '456 Oak Ave' },
    { id: 'PD-101', type: 'Police', status: 'Available', location: 'Sector 4' },
  ];
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Truck size={20}/> Units (Training Simulation)</h2>
      <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-900 border-b border-slate-700">
            <tr>
              <th className="p-3 font-semibold text-slate-400">Unit ID</th>
              <th className="p-3 font-semibold text-slate-400">Type</th>
              <th className="p-3 font-semibold text-slate-400">Status</th>
              <th className="p-3 font-semibold text-slate-400">Location</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700/50">
            {units.map(u => (
              <tr key={u.id} className="hover:bg-slate-700/30">
                <td className="p-3 font-medium text-white">{u.id}</td>
                <td className="p-3 text-slate-400">{u.type}</td>
                <td className="p-3">
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    u.status === 'Available' ? 'bg-green-500/20 text-green-400' :
                    u.status === 'Dispatched' ? 'bg-yellow-500/20 text-yellow-400' :
                    u.status === 'En Route' ? 'bg-blue-500/20 text-blue-400' :
...
function FacsTab() {
  const facs = [
    { name: 'City General Hospital', type: 'Medical', beds: '45 Available', distance: '2.4 mi' },
    { name: 'Mercy Trauma Center', type: 'Medical', beds: '12 Available', distance: '5.1 mi' },
    { name: 'Fire Station 1', type: 'Fire', beds: 'N/A', distance: '1.2 mi' },
    { name: 'Central Police Precinct', type: 'Police', beds: 'N/A', distance: '0.8 mi' },
  ];
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Building2 size={20}/> Facilities (Training Simulation)</h2>
      <div className="grid grid-cols-2 gap-4">
        {facs.map((f, i) => (
          <div key={i} className="bg-slate-800 p-4 rounded-lg border border-slate-700">
            <h3 className="font-bold text-white mb-1">{f.name}</h3>
            <div className="text-sm text-slate-400 grid grid-cols-2 gap-2 mt-3">
              <div><span className="block text-xs text-slate-500 uppercase">Type</span>{f.type}</div>
              <div><span className="block text-xs text-slate-500 uppercase">Distance</span>{f.distance}</div>
              <div className="col-span-2"><span className="block text-xs text-slate-500 uppercase">Capacity</span>{f.beds}</div>
            </div>
          </div>
        ))}
      </div>
...
function SearchTab() {
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Search size={20}/> Database Search (Training Simulation)</h2>
      <div className="max-w-2xl">
        <div className="flex gap-2 mb-6">
          <input type="text" placeholder="Search plates, names, locations, incidents..." className="flex-1 bg-slate-900 border border-slate-700 rounded p-3 text-white focus:outline-none focus:border-blue-500" />
          <button className="bg-blue-600 hover:bg-blue-500 text-white px-6 rounded font-medium transition-colors">Search</button>
        </div>
        <div className="bg-slate-800 rounded-lg border border-slate-700 p-4">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Recent Training Searches</h3>
          <ul className="space-y-2 text-sm text-slate-300">
            <li className="flex items-center gap-2"><Search size={14} className="text-slate-500"/> John Doe (DOB: 05/12/1985)</li>
            <li className="flex items-center gap-2"><Search size={14} className="text-slate-500"/> Lic: ABC-1234 (CA)</li>
            <li className="flex items-center gap-2"><Search size={14} className="text-slate-500"/> 456 Oak Ave (History)</li>
          </ul>
        </div>
      </div>
    </div>
...
function ReportsTab() {
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><FileText size={20}/> Reports (Training Simulation)</h2>
      <div className="grid grid-cols-3 gap-4">
        {['End of Shift Report', 'Incident Summary', 'Unit Utilization', 'Response Times', 'Officer Activity'].map(report => (
          <div key={report} className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-slate-500 cursor-pointer transition-colors">
            <FileText size={24} className="text-blue-400 mb-2" />
            <h3 className="font-medium text-white">{report}</h3>
            <p className="text-xs text-slate-500 mt-1">Generate training mock report</p>
          </div>
        ))}
      </div>
    </div>
...
function SOPTab() {
  const sops = [
    { title: 'Active Shooter Protocol', category: 'Emergency', updated: '2023-11-01' },
    { title: 'Hazmat Spill Response', category: 'Fire/Rescue', updated: '2023-09-15' },
    { title: 'Medical: Cardiac Arrest', category: 'Medical', updated: '2024-01-10' },
    { title: 'Suspicious Package', category: 'Police', updated: '2023-12-05' },
  ];
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><BookOpen size={20}/> Standard Operating Procedures</h2>
      <div className="bg-slate-800 rounded-lg border border-slate-700 overflow-hidden">
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-900 border-b border-slate-700">
            <tr>
              ...
              <th className="p-3 font-semibold text-slate-400">Last Updated</th>
          ...
          <tbody className="divide-y divide-slate-700/50">
            {sops.map((sop, i) => (
              <tr key={i} className="hover:bg-slate-700/30">
                <td className="p-3 font-medium text-white">{sop.title}</td>
                <td className="p-3 text-slate-400">{sop.category}</td>
                <td className="p-3 text-slate-400">{sop.updated}</td>
                <td className="p-3 text-right">
                  <button className="text-blue-400 hover:text-blue-300 text-xs font-medium">View SOP</button>
                </td>
              </tr>
            ))}
          </tbody>
...
function ContactsTab() {
  const contacts = [
    { name: 'Poison Control Center', phone: '1-800-222-1222', desc: 'Toxicology consulting' },
    { name: 'Public Works (Emergency)', phone: '555-0199', desc: 'Water/Sewer/Roads after hours' },
    { name: 'Animal Control', phone: '555-0144', desc: 'Aggressive animals, rescues' },
    { name: 'Coast Guard Sector', phone: '555-0177', desc: 'Marine emergencies' },
  ];
  return (
    <div className="flex-1 p-6 bg-[#0f172a] text-slate-300">
      <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><Phone size={20}/> Important Contacts (Training)</h2>
      <div className="grid grid-cols-2 gap-4 max-w-4xl">
        {contacts.map((c, i) => (
          <div key={i} className="bg-slate-800 p-4 rounded-lg border border-slate-700 flex items-start gap-3">
            <div className="bg-slate-900 p-2 rounded-full text-slate-400 mt-1">
              <Phone size={16} />
            </div>
            <div>
              <h3 className="font-bold text-white text-sm">{c.name}</h3>
              <p className="text-blue-400 font-mono text-sm my-1">{c.phone}</p>
